...
# Shared "install a config on this server" action, parameterised by role so
# the Iran and Outside submenus each get their own sensible default filename.
role_install_config() {
  local default_file="$1" node_file
  read -r -p "Config path [$default_file]: " node_file
  node_file="${node_file:-$default_file}"
  if [[ -f "$node_file" ]]; then
...
role_e2e_probe() {
  ...
  if [[ -f "$client_file" ]]; then
    ...
    printf 'Client config file not found.\n' >&2
  fi
}
...
# Submenu for one server role (iran|outside): install, status, probe, help,
# update, uninstall — everything you do while logged into THAT server.
role_menu() {
  local role="$1" choice label default_file
  if [[ "$role" == iran ]]; then
    label="IRAN SERVER"
    default_file=/root/raah-private-bundle/iran-01.json
  else
    label="OUTSIDE SERVER"
    default_file=/root/raah-private-bundle/outside.json
  fi
  while true; do
    printf '\n----------------------------------------\n'
    printf 'RAAH | %s\n' "$label"
    printf '----------------------------------------\n'
    printf '1) Install/start config on this server (%s)\n' "$default_file"
    printf '2) Service status\n'
    printf '3) End-to-end client probe\n'
    printf '4) Help\n'
    printf '5) Update Raah to the latest stable release\n'
    printf '6) Uninstall Raah completely from this server\n'
    printf '0) Back\n'
    printf '----------------------------------------\n'
    read -r -p 'Select [0-6]: ' choice || return 0
    case "$choice" in
      1) role_install_config "$default_file" ;;
      2) systemctl --no-pager status raah-sing-box raah-port-hop || true ;;
      3) role_e2e_probe ;;
      4) usage ;;
      5) role_update ;;
      6) uninstall_raah; return 0 ;;
      0) return 0 ;;
...
# Submenu for the setup/build host: this is where the shared bundle (both
# servers' configs) gets generated or edited. It is independent of whether
# this particular machine ends up being Iran, Outside, or neither.
bundle_menu() {
  local choice bundle_path
  while true; do
    printf '\n----------------------------------------\n'
    printf 'RAAH | BUNDLE GENERATOR (setup host)\n'
    printf '----------------------------------------\n'
    ...
    printf '4) Safely edit an existing bundle\n'
    printf '0) Back\n'
    printf '----------------------------------------\n'
    read -r -p 'Select [0-4]: ' choice || return 0
    case "$choice" in
      1|2|3)
      ...
      0) return 0 ;;
      *) printf 'Invalid option. Enter a number from 0 to 4.\n' >&2 ;;
    esac
  done
}
...
menu() {
  ...
  while true; do
    ...
    printf '2) Outside server\n'
    printf '3) Generate / edit config bundle (setup host)\n'
    printf '0) Exit\n'
    printf '========================================\n'
    read -r -p 'Select [0-3]: ' choice || return 0
    case "$choice" in
      1) role_menu iran ;;
      2) role_menu outside ;;
      3) bundle_menu ;;
      0) return 0 ;;
      *) printf 'Invalid option. Enter a number from 0 to 3.\n' >&2 ;;
    esac
  done
}
